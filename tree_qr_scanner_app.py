...
def decode_qr_camera(image_bytes):
    try:
...
# QR Input Section
st.header("1. QR Code Input")
tabs = st.tabs(["Upload QR Image", "Use Camera"])
...
with tabs[1]:
    qr_captured = st.camera_input("📸 Capture QR Code")
    if qr_captured:
        qr_result = decode_qr_camera(qr_captured.read())
        if qr_result:
            st.session_state.qr_result = qr_result
            st.success(f"QR Code Found: {qr_result}")
        else:
            st.error("No QR code detected in the captured image.")
